...
class DatabaseManager:
    ...
    def insert_value(self, symbol, date, value):
        self.cursor.execute("INSERT INTO rrg_data (symbol, date, value) VALUES (%s, %s, %s)", (symbol, date, value))

    def insert_price_relative(self, symbol, date, value):
        self.cursor.execute("UPDATE rrg_data SET price_relative = %s WHERE symbol = %s AND date = %s", (value, symbol, date))

    def get_data(self, symbol):
        self.cursor.execute("SELECT * FROM rrg_data WHERE symbol = %s ORDER BY date DESC", (symbol,))
